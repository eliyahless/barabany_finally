...
async function verifyFormToken(formId: string, timestamp: number, token: string): Promise<boolean> {
  ...
  // Проверяем сам токен
  const expectedToken = await generateFormToken(formId, timestamp)
...
export async function submitContactForm(formData: FormData): Promise<FormResult> {
  try {
    ...
    if (rawData.formId && rawData.timestamp && rawData.token) {
      if (!(await verifyFormToken(rawData.formId, rawData.timestamp, rawData.token))) {
        return {
          success: false,
